...
def _write_summary_via_report(out_dir: Path, results_file: Path, seeds: list[int]) -> Path | None:
    """Delegate RESULTS.md to scripts.report so there is exactly ONE generator.

    Two table writers is how the JSON and the document drift apart -- the
    failure mode task 3 exists to fix. Mid-sweep the metrics directory is
    incomplete by definition, so this runs non-strict (missing runs are simply
    absent from the table) and never aborts the sweep over a report problem.
    """
    from scripts.report import ReportError, write_report  # noqa: PLC0415

    try:
        return write_report(out_dir, results_file, strict=False, seeds_hint=seeds)
    except ReportError as exc:
        log.warning("could not write %s yet: %s", paths.rel(results_file), exc)
        return None
...
def main(argv: list[str] | None = None) -> int:
    ...
    for i, (tier, seed) in enumerate(matrix, start=1):
        ...
        _write_summary_via_report(out_dir, results_file, seeds)

    csv_path, md_path = write_results(records, out_dir, started=started)
    summary_path = _write_summary_via_report(out_dir, results_file, seeds) or results_file
